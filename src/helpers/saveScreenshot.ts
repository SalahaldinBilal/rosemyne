 ...
 */
export async function saveScreenshot(id: number, position: Dimensions, image: ImageData, playSound = true): Promise<void> {
  await invoke("hide_and_save_screenshot", new Uint8Array(image.data.buffer, image.data.byteOffset, image.data.byteLength), {
    headers: {
      "x-rosemyne-args": JSON.stringify({ id, position, width: image.width, height: image.height, playSound }),
    },
  });
}
